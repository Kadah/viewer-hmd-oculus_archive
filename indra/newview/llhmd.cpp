...
void LLHMD::setRenderMode(U32 mode, bool setFocusWindow)
{
    ...
    if (newRenderMode != mRenderMode)
    {
		...
		windowp->setHMDMode(isHMDMode(), gHMD.getViewportWidth(), gViewerWindow->getWindowHeightRaw());
...
void LLHMD::calculateMouseWorld(S32 mouse_x, S32 mouse_y, LLVector3& world)
{
    ...
    else
    {
		...
		F32 ha = ((mUIShape.mArcVertical * -0.5f) + (nx * mUIShape.mArcVertical));
		F32 va = (F_PI - (mUIShape.mArcHorizontal * 0.5f)) + (ny * mUIShape.mArcHorizontal);
...
void LLHMD::setupStereoCullFrustum()
{
    ...
    cam->setPerspective(!FOR_SELECTION, 0, 0, getViewportWidth() * 3, getViewportHeight(), FALSE, cam->getNear(), cam->getFar());
}
